...
def generate_cloud(PATH_IMG_L : str, PATH_IMG_R : str, akaze_t : float = 0.0001, lowe : float = 0.8) :
    ...
    sift = cv.SIFT_create(contrastThreshold=0.05, edgeThreshold=10, nOctaveLayers=4)
    ...
    if len(pts_g_brut) > 10:
        good_mask = [True]*len(pts_g_brut) # Endroit pour mettre un masque à l'issue
    ...
    mask = (points3D[:, 2] > 0.01) & (points3D[:, 2] < 5.0)     # réduit à 5m (problème #2)
    p3d = points3D[mask]
...
if __name__ == '__main__' :
    ...
    parser.add_argument('--gauche', help='Chemin image gauche', default='SLAM/data_sortie_mer/frames/gauche/sortie_left/frames/frame_002409.jpg')
    parser.add_argument('--droite', help='Chemin image droite', default='SLAM/data_sortie_mer/frames/droite/sortie_right/frames/frame_002409.jpg')
