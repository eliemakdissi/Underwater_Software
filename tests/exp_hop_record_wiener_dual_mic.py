...
class EnregistreurHydrophoneWienerDualMic(QMainWindow):
    def __init__(self):
        ...
        lbl_in_ch1 = QLabel("In CH1:")
        lbl_in_ch1.setFont(pc)
        self.combo_in_ch1 = QComboBox()
        self.combo_in_ch1.setFont(pc)

        lbl_in_ch2 = QLabel("In CH2:")
        lbl_in_ch2.setFont(pc)
        self.combo_in_ch2 = QComboBox()
        self.combo_in_ch2.setFont(pc)
        ...
        add_input_output_devices(self.combo_in_ch1, self.combo_out)
        select_default_input_output_devices(self.combo_in_ch1, self.combo_out)
        for idx in range(self.combo_in_ch1.count()):
            self.combo_in_ch2.addItem(
                self.combo_in_ch1.itemText(idx),
                self.combo_in_ch1.itemData(idx),
            )
        if self.combo_in_ch1.currentIndex() >= 0:
            self.combo_in_ch2.setCurrentIndex(self.combo_in_ch1.currentIndex())
        ...
        self.plot_spectro_ch1.setYRange(0, self.samplerate / 2000, padding=0)
        ...
        self.plot_spectro_ch2.setYRange(0, self.samplerate / 2000, padding=0)
    ...
    def callback_audio(self, indata, outdata, frames, time, status):
        ...
        if self.stream:
            ch1 = data_proc[:, 0].astype(np.float32, copy=False)
            ch2 = data_proc[:, 1].astype(np.float32, copy=False) if data_proc.shape[1] > 1 else ch1
            self.audio_queue_ch1.put(ch1.copy())
            self.audio_queue_ch2.put(ch2.copy())
            self.session_waveform_ch1.append(ch1.copy())
            self.session_waveform_ch2.append(ch2.copy())
    ...
    def update_charts(self):
        ...
        if new_data_ch1 is not None:
            ...
            self.img_spectro_ch1.setRect(QRectF(0, 0, self.duree_visible, self.samplerate / 2000))
        ...
        if new_data_ch2 is not None:
            ...
            self.img_spectro_ch2.setRect(QRectF(0, 0, self.duree_visible, self.samplerate / 2000))
    ...
    def demarrer_enregistrement(self):
        ...
        idx_in_ch1 = self.combo_in_ch1.currentData()
        idx_in_ch2 = self.combo_in_ch2.currentData()
        idx_out = self.combo_out.currentData()

        try:
            if idx_in_ch1 != idx_in_ch2:
                raise ValueError(
                    "Two different input devices are not supported in one stream. "
                    "Please select the same device for In CH1 and In CH2."
                )
            input_info = sd.query_devices(idx_in_ch1)
            ...
            self.combo_in_ch1.setEnabled(False)
            self.combo_in_ch2.setEnabled(False)
    ...
    def arreter_enregistrement(self):
        ...
        self.combo_in_ch1.setEnabled(True)
        self.combo_in_ch2.setEnabled(True)
